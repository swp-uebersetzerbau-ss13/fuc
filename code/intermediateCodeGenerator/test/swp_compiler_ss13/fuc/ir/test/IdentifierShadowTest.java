...
public class IdentifierShadowTest {
	...
	public void setUp() throws Exception {
		...
		astf.addAssignment(astf.newBasicIdentifier("shadow"), astf.newLiteral("true", new BooleanType()));
		astf.addPrint(astf.newBasicIdentifier("shadow"));
		astf.addBlock();
		astf.addDeclaration("shadow", new LongType());
		astf.addAssignment(astf.newBasicIdentifier("shadow"), astf.newLiteral("25", new LongType()));
		astf.addPrint(astf.newBasicIdentifier("shadow"));
		astf.addBlock();
		astf.addDeclaration("shadow", new StringType(10L));
		astf.addAssignment(astf.newBasicIdentifier("shadow"), astf.newLiteral("Hallo Welt", new StringType(10L)));
		astf.addPrint(astf.newBasicIdentifier("shadow"));
		astf.goToParent();
		astf.addPrint(astf.newBasicIdentifier("shadow"));
		astf.goToParent();
