...
public class ParserImpl implements Parser {
	...
	protected AST parse() {
		...
		while (true) {
			s = this.parserStack.peek();

			tokenType = token.getTokenType();

			if (tokenType == TokenType.NOT_A_TOKEN) {
				this.reportLog.reportError(token.getValue(), token.getLine(), token.getColumn(),
						"Undefined Symbol found!");
				return null;
			}

			entry = this.table.getActionEntry(s, new Terminal(token));

			if (entry == null) {
				this.log.error("Error in Parsetable occured!");
				return null;
			}
			...
			case SHIFT: {
				...
				// Isn't it better to push a token, otherwise there are only
				// null and NOVALUE onto stack?
			...
			case REDUCE: {
				...
				ReduceAction reduceAction = this.getReduceAction(prod);

				// check where to go-to... and push next state on stack
				GotoEntry gotoEntry = this.table.getGotoEntry(s, prod.getLeft());
				this.parserStack.push(gotoEntry.getNewState());
				...
				if (reduceAction != null) {
					...
					for (int i = 0; i < nrOfValuesReduced; i++) {
						valueHandle.addFirst(this.valueStack.pop());
					}

					// Execute reduceAction and push onto the stack
					Object newValue = reduceAction.create(arr(valueHandle));
					if (newValue == null) {
						this.log.error("Error occurred! newValue is null");
						return null;
					}
					this.valueStack.push(newValue);
				}
			}
			...
			case ACCEPT: {
				if (tokenType != TokenType.EOF) {
					...
					BlockNode programBlock = (BlockNode) this.valueStack.pop();
					this.ast.setRootNode(programBlock);
					programBlock.setParentNode(this.ast.getRootNode());
					return this.ast;
				}
			}
			...
			case ERROR: {
				...
				this.reportLog.reportError(token.getValue(), token.getLine(), token.getColumn(), "An error occurred: "
						+ error.getMsg());
				...
				public Object create(Object... objs) {
					...
					if (!left.equals(NO_VALUE)) {
						...
						if (!(left instanceof DeclarationNode)) {
							ParserImpl.this.log.error("Error in decls -> decls decl: Left must be a DeclarationNode!");
						} else {
					...
					if (right instanceof BlockNode) {
						...
						if (!(right instanceof DeclarationNode)) {
							ParserImpl.this.log.error("Error in decls -> decls decl: Right must be a DeclarationNode!");
						} else {
			...
			return new ReduceAction() {
				...
				public Object create(Object... objs) {
					...
					if (token.getTokenType() == TokenType.ID) {
						...
						ParserImpl.this.log.error("Wrong TokenType in ReduceAction \"loc -> id\"");
						return null;
					}
