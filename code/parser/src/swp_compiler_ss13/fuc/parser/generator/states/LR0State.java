...
public class LR0State extends ALRState<LR0Item> {
	...
	public LR0State goTo(Symbol symbol) {
		LinkedHashSet<LR0Item> ret = new LinkedHashSet<LR0Item>();
	...
	/**
	 * @param grammarInfo
	 * @return The closure for this state as a new {@link LR0State}
	 */
	private LR0State calcClosure(GrammarInfo grammarInfo) {
		// The closure contains all items of the source...
		LinkedHashSet<LR0Item> result = new LinkedHashSet<LR0Item>();
		result.addAll(items);

		// ... and the following ones (for any item "A → α.Xβ" and any "X → γ"
		// add "X → .γ")
		...
		while (!queue.isEmpty()) {
			...
			if (it.isShiftable() && nextSymbol.isNonTerminal()) {
				for (Production p : grammarInfo
						.getProductionsFrom((NonTerminal) nextSymbol)) {
