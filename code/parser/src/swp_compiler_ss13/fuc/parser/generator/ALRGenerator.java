...
public abstract class ALRGenerator<I extends Item, S extends ALRState<I>> {
   ...
   public ALRGenerator(Grammar grammar) throws RuntimeException {
      ...
      table = new LRParsingTable(dfa.getEdges().size());
      ...
      statesMap.put(startState, tableStates.next());
      for (S dfaState : dfa.getStates()) {
         if (!dfaState.equals(startState)) {
            statesMap.put(dfaState, tableStates.next());
         }
      }
         ...
         if (edge.getSymbol().isTerminal()) {
            ...
            if (edge.isDestAccepting()) {
               ...
               try {
                  ...
                  throw new RuntimeException(err); // TODO Really the right way...?
               }
            } else {
               // SHIFT
      ...
      for (S kernel : statesMap.keySet()) {
         ...
         for (I item : state.getItems()) {
            if (item.isComplete()) {
               ...
               // Aw! I need to add this Reduce for all elements of the FOLLOW-set of this item...
               ITerminalSet terminalSet = grammarInfo.getFollowSets().get(item.getProduction().getLHS());
               for (Terminal terminal : terminalSet.getTerminals()) {
                  try {
                     table.getActionTable().set(new Reduce(item.getProduction()), fromState, terminal);
                  } catch (DoubleEntryException err) {
                     throw new RuntimeException(err); // TODO Really the right way...?
                  }
               }
