...
public class FollowSets {
   ...
   // --- constructors ---------------------------------------------------------
   // --------------------------------------------------------------------------
   public FollowSets(Grammar grammar, FirstSets firstSets, NullableSet nullableSet) {
      ...
      do {
         for (Production prod : grammar.getProductions()) {
            ...
            ListIterator<Symbol> itRight = prod.getRHS().listIterator(prod.getRHS().size());
            EfficientTerminalSet FOLLOWToAdd = getFollowSet(lhs);
            
            while (itRight.hasPrevious()) { // right (step through right side of the production)
               Symbol right = itRight.previous();
               if (right.isTerminal()) {
                  // TERMINAL: Clear FOLLOW, add it as only element
                  FOLLOWToAdd = FOLLOWToAdd.empty().plus((Terminal) right);
                  ...
                  EfficientTerminalSet rightFollowNew = addToFollow(varRight, FOLLOWToAdd);
                  // iCard += (varRight.getFOLLOW().size() - oldSize);
                  changed |= (!rightFollowNew.equals(rightFollowOld)); // Changed anything?
                  
                  // 2. prepare new FOLLOWToAdd:
                  if (!nullableSet.contains(varRight)) {
                     FOLLOWToAdd = FOLLOWToAdd.empty();
                  } else {
                     // If varRight can produce EPSILON (= isNullable()), we keep our follow-set and try to add it to
                     // the next (Non?)Terminal on the left
                  }
                  
//                  Set<Terminal> currentFIRST = new HashSet<Terminal>(varRight.getFIRST());
                  EfficientTerminalSet curFirst = firstSets.get(varRight);
//                currentFIRST.remove(eps);
                  curFirst = curFirst.empty().plusAllExceptEpsilon(curFirst);
...
//                  } ==> see above!
   ...
   private EfficientTerminalSet addToFollow(NonTerminal key, EfficientTerminalSet terminalSet) {
      EfficientTerminalSet set = getFollowSet(key);
      set = set.plusAll(terminalSet);
      followSets.put(key, set); // Needed, as TerminalSets are immutable and plus(..) creates new instance
      return set;
   }
