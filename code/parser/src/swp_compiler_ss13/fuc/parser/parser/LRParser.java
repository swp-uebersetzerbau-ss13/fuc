...
public class LRParser {
	...
	public AST parse(LexerWrapper lexer, ReportLog reportLog,
		...
		WHILE: while (true) {
			...
			case REDUCE: {
				...
				reduceAction = this.getReduceAction(prod, reportLog);
	...
	private ReduceAction getReduceAction(Production prod, final ReportLog reportLog) {
		...
		case "decls -> decls decl":
			return new ReduceAction() {
				@Override
				public Object create(Object... objs) throws ParserException {
					Object left = objs[0]; // Should be NO_VALUE, BlockNode or
											// DeclarationNode
					...
					if (!left.equals(NO_VALUE)) {
						if (left instanceof BlockNode) {
							...
							LRParser.this.log.error("Error in decls -> decls decl: Left must be a DeclarationNode!");
						} else {
					...
					if (right instanceof BlockNode) {
						...
						if (!(right instanceof DeclarationNode)) {
							LRParser.this.log.error("Error in decls -> decls decl: Right must be a DeclarationNode!");
						} else {
		...
		case "stmts -> stmts stmt":
			return new ReduceAction() {
				@Override
				public Object create(Object... objs) throws ParserException {
					Object left = objs[0]; // Should be NO_VALUE or BlockNode or
											// other
											// StatementNode
			...
			return new ReduceAction() {
				...
				public Object create(Object... objs) throws ParserException {
					...
					if (token.getTokenType() == TokenType.ID) {
						...
						LRParser.this.log.error("Wrong TokenType in ReduceAction \"loc -> id\"");
						return null;
					}
		...
		case "assign -> loc = assign":
			return new ReduceAction() {
				...
				public Object create(Object... objs) throws ParserException {
					...
					assignNode.setRightValue((ExpressionNode) objs[2]); // [1]
																		// is
																		// the
																		// "="
																		// token
	...
	/**
	 * Inserts the {@link DeclarationNode} into the block and its
	 * {@link SymbolTable} safely.
	 * 
	 * @param block
	 * @param decl
	 */
	protected static void insertDecl(BlockNode block, DeclarationNode decl, final ReportLog reportLog)
